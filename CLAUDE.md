...
| Illustration (animated) | [collection ↗](https://diegoalo.github.io/liferay-tests/empty-state-animations.html) | Decorative empty-state art by Diego Alonso, rebuilt as **inline SVG** (not `<img>` data-URIs). Geometry stays as SVG paths; every fill/stroke/gradient-stop is a token via `.f-*` / `.s-*` utility classes (each sets both `fill` and `stop-color`, so one class works on a `<path>` or a `<stop>`) → the art reskins across the four skins. All motion is page CSS in `components.css` — never inside the SVG — so a single native `@media (prefers-reduced-motion: reduce)` rule stills every illustration, with **no static-image fallback and no JS toggle**. Reusable keyframes `il-breathe` / `il-twinkle` / `il-sway` plus per-variant blocks; animated parts carry `.il-*` / `.esNN-*` hooks. Variants are `.illustration--<name>` (blue *Producto* set: rocket, satellite, telescope, pagebuilder, contenttypes, toolbar-canvas, discovery, document, audience, workflow, toolbar-list, action-toolbar). Gradient ids are prefixed per variant to stay unique when several are inlined. Inside an `.empty-state`, the media's tinted circle is auto-dropped (`:has(.illustration)`). Gallery + skin switcher in `showcases/illustration.html`. *(AI Hub / purple set — es14–es18 — pending.)* |
